...
public final class LogUtils {
    ...
    private static void printDeviceInfo(final String filePath) {
        ...
        int versionCode = 0;
        try {
            PackageInfo pi = AppUtils.Companion.getApp()
                    .getPackageManager()
                    .getPackageInfo(AppUtils.Companion.getApp().getPackageName(), 0);
            if (pi != null) {
    ...
    public static class Config {
        private Config() {
            ...
            if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())
                    && AppUtils.Companion.getApp().getExternalCacheDir() != null)
                sDefaultDir = AppUtils.Companion.getApp().getExternalCacheDir() + FILE_SEP + "log" + FILE_SEP;
            else {
                sDefaultDir = AppUtils.Companion.getApp().getCacheDir() + FILE_SEP + "log" + FILE_SEP;
            }
        }
